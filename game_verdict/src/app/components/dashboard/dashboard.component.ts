  ...
  export class DashboardComponent implements OnInit {
    ...
    onSubmitGame(): void {
      if (this.isEditingGame && this.currentGameId !== null) {
        ...
        const platformIds = this.newGame.platforms.map(platform => platform.id);
        const genreIds = this.newGame.genres.map(genre => genre.id);
        ...
        this.gameService.updateGame(this.currentGameId, gameToUpdate).subscribe({
          next: () => {
            this.loadGames();
            this.resetGameForm();
    ...
    editGame(game: Game): void {
      ...
      const selectedPlatforms = this.platforms.filter(platform =>
        game.platforms.some(gamePlatform => gamePlatform.id === platform.id)
      );

      const selectedGenres = this.genres.filter(genre =>
